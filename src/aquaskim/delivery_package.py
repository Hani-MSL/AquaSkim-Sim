...
only the public final entrypoints, not internal development scripts.
"""
...
def _validate_prerequisites() -> dict[str, Any]:
    ...
    checks = {
        ...
        "curated_media_ge_12_pairs": int(curation_manifest.get("selected_gifs", 0)) >= 12
        and int(curation_manifest.get("selected_mp4s", 0)) >= 12,
        "versions_match": __version__ == config.project_version,
        "manifest_hashes_match_docx": bool(
            version_checks["word_manifest_report_sha_matches_docx"]
            and version_checks["word_qa_sha_matches_docx"]
        ),
    }
    if not all(checks.values()):
        failing = {key: value for key, value in checks.items() if not value}
        raise FinalDeliveryError(
            f"Final delivery prerequisites failed: {json.dumps(failing, ensure_ascii=False)}"
        )
    return {
        "checks": checks,
...
def _validate_reproduction_script_set() -> list[dict[str, Any]]:
        ...
        rows.append(
            {
                ...
                "exists": exists,
                "size_bytes": size,
                "sha256": _sha256(path) if exists and size > 0 else "",
            }
        )
    ...
    if missing_or_empty:
        raise FinalDeliveryError(
            "Required reproduction entrypoints are missing or empty: "
...
def _delivery_files() -> list[DeliveryFile]:
    ...
    add(
        DeliveryFile(
            DIRECTORIES["reports"] / "AquaSkim-Sim_Final_Report.docx",
    ...
    for pattern in (
        ...
        for path in deliverables.glob(pattern):
            try:
                ...
                raise FinalDeliveryError(
                    f"Cannot remove stale delivery artifact; close viewers and retry: {relative_to_root(path)}"
                ) from exc
...
def build_final_delivery_package(*, record: bool = True) -> dict[str, Any]:
    ...
    internal_manifest = {
        ...
        "scope": (
            "Final project delivery package assembled from the validated Word report "
            "and curated reference evidence."
        ),
        "model_boundary": (
            "Numerical low-speed 3-DOF sheltered-basin evidence only; not sea-trial footage, "
            "certification, wave-response evidence, or onboard current-estimation proof."
        ),
    ...
    temp_readme.write_text(
        ...
        "- Evidence source: curated fixed-reference simulation outputs.\n"
    ...
    if not verification["passed"]:
        raise FinalDeliveryError(
            f"Delivery ZIP verification failed: {json.dumps(verification, ensure_ascii=False)}"
        )
    ...
    sums_lines.extend(f"{row['sha256']}  {row['path']}" for row in file_rows)
...
def _verify_package(package_zip: Path, file_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    with zipfile.ZipFile(package_zip) as archive:
        ...
        forbidden = [
            name
            for name in names
            if "backup" in Path(name).name.lower()
            or name.endswith(".pyc")
            or "__pycache__" in name
            or any(part in EXCLUDED_PARTS for part in Path(name).parts)
            or Path(name).name.startswith(EXCLUDED_NAME_PREFIXES)
        ]
    checks = {
...
def _audit_markdown(manifest: dict[str, Any]) -> str:
    ...
    lines.extend(f"- {claim}" for claim in manifest["explicit_non_claims"])
    return "\n".join(lines) + "\n"
...
def _record_run(manifest: dict[str, Any], audit_md: str) -> None:
    ...
    (run_dir / "FINAL_DELIVERY_PACKAGE_MANIFEST.json").write_text(
    ...
    handoff.write_text(
        ...
        "- This is a project delivery package, not a sea-trial or certification package.\n"
